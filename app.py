...
LANG_UI = {
    'EL': {
        ...
        'tab_emotions': "🎭 Συναισθηματικό Μητρώο",
        ...
        'emotions_sub': "🎭 Συναισθηματική Ανάλυση του Origin-based Corpus",
        'emotions_note': "Η ανάλυση βασίζεται σε AI-assisted row-level scoring των εγγραφών με τεκμηριωμένο news_origin_norm. Οι βαθμολογίες αποτυπώνουν το ρητορικό/συναισθηματικό μητρώο του ιστορικού κειμένου, όχι την πραγματική ένταση των γεγονότων.",
        'emotions_total': "Εγγραφές",
        'emotions_top': "Κυρίαρχο Συναίσθημα",
        'emotions_error': "Σφάλματα",
        'emotions_mean': "Μέσος Δείκτης",
        'emotions_dominant_counts': "Κατανομή Κυρίαρχου Συναισθήματος",
        'emotions_means_overall': "Μέσοι Όροι Συναισθημάτων",
        'emotions_by_year': "Εξέλιξη Συναισθημάτων ανά Έτος",
        'emotions_by_country': "Συναισθήματα ανά Χώρα",
        'emotions_by_frame': "Συναισθήματα ανά Ρητορικό Πλαίσιο",
        'emotions_by_event_type': "Συναισθήματα ανά Τύπο Γεγονότος",
        'emotions_filter_group': "Ομαδοποίηση:",
        'unknown': "Άγνωστο",
    ...
    'EN': {
        ...
        'tab_emotions': "🎭 Emotional Register",
        ...
        'emotions_sub': "🎭 Emotional Analysis of the Origin-based Corpus",
        'emotions_note': "This analysis is based on AI-assisted row-level scoring of records with documented news_origin_norm. Scores capture the rhetorical/emotional register of the historical text, not the actual intensity of events.",
        'emotions_total': "Records",
        'emotions_top': "Dominant Emotion",
        'emotions_error': "Errors",
        'emotions_mean': "Mean Score",
        'emotions_dominant_counts': "Dominant Emotion Distribution",
        'emotions_means_overall': "Mean Emotion Scores",
        'emotions_by_year': "Emotion Evolution by Year",
        'emotions_by_country': "Emotions by Country",
        'emotions_by_frame': "Emotions by Rhetorical Frame",
        'emotions_by_event_type': "Emotions by Event Type",
        'emotions_filter_group': "Group by:",
        'unknown': "Unknown",
    ...
    'FR': {
        ...
        'tab_emotions': "🎭 Registre émotionnel",
        ...
        'emotions_sub': "🎭 Analyse émotionnelle du corpus fondé sur les origines",
        'emotions_note': "Cette analyse repose sur une notation assistée par IA, au niveau de chaque entrée, des notices disposant d’un news_origin_norm documenté. Les scores indiquent le registre rhétorique/émotionnel du texte historique, non l’intensité réelle des événements.",
        'emotions_total': "Entrées",
        'emotions_top': "Émotion dominante",
        'emotions_error': "Erreurs",
        'emotions_mean': "Score moyen",
        'emotions_dominant_counts': "Distribution de l’émotion dominante",
        'emotions_means_overall': "Scores émotionnels moyens",
        'emotions_by_year': "Évolution des émotions par année",
        'emotions_by_country': "Émotions par pays",
        'emotions_by_frame': "Émotions par cadre rhétorique",
        'emotions_by_event_type': "Émotions par type d’événement",
        'emotions_filter_group': "Regrouper par:",
        'unknown': "Inconnu",
...
def load_thesis_data_v4():
    try:
        ...
        if 'newspaper_title' not in df.columns:
            ...
            if possible_names:
                df = df.rename(columns={possible_names[0]: 'newspaper_title'})
...
def load_emotions_data():
    try:
        file_path = os.path.join(BASE_DIR, "news_wave_emotions_streamlit_slim.csv")
        df = pd.read_csv(file_path, low_memory=False)
        df.columns = df.columns.str.lower().str.strip()
        ...
        if "year" in df.columns:
            ...
            df["year_val"] = pd.to_numeric(
                df["date"].astype(str).str.extract(r"(18[23]\d)")[0],
                errors="coerce"
            )
        else:
            df["year_val"] = None

        if "country_corrected_safe" in df.columns:
            df["country_display"] = df["country_corrected_safe"].astype(str).str.strip().str.upper().replace({
                "UK": "GB",
                "UNITED KINGDOM": "GB",
                "FRANCE": "FR"
            })
        elif "country" in df.columns:
            df["country_display"] = df["country"].astype(str).str.strip().str.upper().replace({
                "UK": "GB",
                "UNITED KINGDOM": "GB",
                "FRANCE": "FR"
            })
        else:
            df["country_display"] = "Unknown"

        return df
    except Exception as e:
        st.error(f"Error loading emotions data: {e}")
        return pd.DataFrame()
...
df_emotions = load_emotions_data()
...
with t1:
    ...
    with c_pie1:
        ...
        if not raw_relevance.empty:
            fig_p = px.pie(
                values=raw_relevance.values,
                names=raw_relevance.index,
                hole=0.4,
...
with t2:
    ...
    if 'newspaper_title' in df_filt.columns and 'ai_stance' in df_filt.columns:
        ...
        df_np = df_temp[df_temp['newspaper_title'].isin(top_np)].groupby(
            ['newspaper_title', 'ai_stance']
...
with t3:
    ...
    if 'ai_topic' in df_filt.columns:
        ...
        df_t = valid_topics_time[valid_topics_time['ai_topic'].isin(top_t)].groupby(
            ['year_val', 'ai_topic']
        ).size().reset_index(name='count')

        fig_topics = px.area(df_t, x='year_val', y='count', color='ai_topic', height=500)
        st.plotly_chart(fig_topics, use_container_width=True)

# ==========================================
# ΚΑΡΤΕΛΑ 4: ΡΟΕΣ & ΧΑΡΤΗΣ
# ==========================================
with t4:
    ...
    if c_src and c_dst:
        ...
        f_df = f_df[
            (~f_df[c_src].astype(str).str.lower().isin(bad_words)) &
            (~f_df[c_dst].astype(str).str.lower().isin(bad_words))
        ]
        ...
        if not f_df.empty:
            ...
            fig_s = go.Figure(go.Sankey(
                ...
                link=dict(
                    source=f_grp[c_src].map(mapping),
            ...
            if fr_lon:
                fig_map.add_trace(go.Scattergeo(
                    ...
                    lat=fr_lat,
                    mode='lines',
                    line=dict(width=1.5, color='#ff4d4d'),
                    opacity=0.5,
                    name=ui['map_legend_fr'],
                    hoverinfo='skip'
                ))
...
with t5:
    ...
    def make_ner_chart(col_name, color, title):
        if col_name in df_filt.columns:
            data = df_filt[col_name].astype(str).str.split(',').explode().str.strip().replace('', pd.NA).dropna()
            if not data.empty:
...
with t6:
    ...
    if df_waves.empty or not wave_cards:
        ...
        if card_data:
            ...
            c1.metric(ui['waves_records'], card_data.get('count', 0))
            c2.metric(ui['waves_frame'], card_data.get('dominant_frame', '-'))
            c3.metric(ui['waves_type'], card_data.get('dominant_event_type', '-'))
            c4.metric(ui['waves_medium'], card_data.get('dominant_transmission_medium', '-'))
...
# ==========================================
# ΚΑΡΤΕΛΑ 7: EMOTIONAL REGISTER
# ==========================================
with t7:
    st.subheader(ui['emotions_sub'])
    st.info(ui['emotions_note'])

    if df_emotions.empty:
        st.error("⚠️ Λείπει το αρχείο 'news_wave_emotions_streamlit_slim.csv'.")
    else:
        emotion_cols = [
            "emotion_fear",
            "emotion_pity",
        ...
        df_e = df_emotions.copy()

        # Εφαρμογή φίλτρων sidebar
        if "country_display" in df_e.columns:
            df_e = df_e[df_e["country_display"].isin(sel_countries)]

        if "year_val" in df_e.columns:
            df_e = df_e[
                (df_e["year_val"] >= sel_years[0]) &
                (df_e["year_val"] <= sel_years[1])
            ]

        # Metrics
        c1, c2, c3, c4 = st.columns(4)

        c1.metric(ui['emotions_total'], f"{len(df_e):,}")

        if "emotion_dominant" in df_e.columns and not df_e.empty:
            top_emotion = df_e["emotion_dominant"].fillna("Unknown").astype(str).value_counts().idxmax()
            c2.metric(ui['emotions_top'], top_emotion)
        else:
            c2.metric(ui['emotions_top'], "-")

        if "emotion_error" in df_e.columns:
            err_mask = ~df_e["emotion_error"].fillna("").astype(str).str.strip().str.lower().isin(["", "nan", "none", "null"])
            c3.metric(ui['emotions_error'], int(err_mask.sum()))
        else:
            c3.metric(ui['emotions_error'], "-")

        if not df_e.empty:
            mean_all = df_e[emotion_cols].mean().mean()
            c4.metric(ui['emotions_mean'], f"{mean_all:.2f}")
        else:
            c4.metric(ui['emotions_mean'], "-")

        st.divider()

        # 1. Dominant emotion counts + mean scores
        col1, col2 = st.columns(2)

        with col1:
            st.markdown(f"**{ui['emotions_dominant_counts']}**")

            if "emotion_dominant" in df_e.columns and not df_e.empty:
                dom = df_e["emotion_dominant"].fillna("Unknown").astype(str).str.strip().replace("", "Unknown")
                dom_counts = dom.value_counts().reset_index()
        ...
        with col2:
            st.markdown(f"**{ui['emotions_means_overall']}**")

            means = df_e[emotion_cols].mean().reset_index()
            means.columns = ["Emotion", "Mean"]
            means["Emotion"] = means["Emotion"].map(emotion_labels)
            ...
            fig_means.update_layout(
                height=420,
                margin=dict(t=20, b=20, l=10, r=10),
                yaxis_title=ui['emotions_mean']
            )
            st.plotly_chart(fig_means, use_container_width=True)

        st.divider()

        # 2. Emotion evolution by year
        st.markdown(f"**{ui['emotions_by_year']}**")

        if "year_val" in df_e.columns and not df_e.empty:
            year_means = df_e.groupby("year_val")[emotion_cols].mean().reset_index()
            year_long = year_means.melt(
                id_vars="year_val",
                value_vars=emotion_cols,
                var_name="Emotion",
                value_name="Mean"
            )
            year_long["Emotion"] = year_long["Emotion"].map(emotion_labels)
        ...
        # 3. Grouped emotion analysis
        group_options = {}

        if "country_display" in df_e.columns:
            group_options[ui['emotions_by_country']] = "country_display"
        if "rhetorical_frame_primary" in df_e.columns:
            group_options[ui['emotions_by_frame']] = "rhetorical_frame_primary"
        if "canonical_event_type" in df_e.columns:
            group_options[ui['emotions_by_event_type']] = "canonical_event_type"
        if "rumor_status" in df_e.columns:
            group_options[ui['waves_rumor']] = "rumor_status"
        if "transmission_medium" in df_e.columns:
            group_options[ui['waves_medium']] = "transmission_medium"
        if "event_phase" in df_e.columns:
            group_options[ui['waves_phase']] = "event_phase"

        if group_options:
            selected_group_label = st.selectbox(
                ui['emotions_filter_group'],
                list(group_options.keys())
            )
            selected_group = group_options[selected_group_label]

            group_df = df_e.copy()
            group_df[selected_group] = group_df[selected_group].fillna("Unknown").astype(str).str.strip().replace("", "Unknown")

            counts = group_df[selected_group].value_counts()
            valid_groups = counts[counts >= 20].index
            group_df = group_df[group_df[selected_group].isin(valid_groups)]

            if not group_df.empty:
                grouped = group_df.groupby(selected_group)[emotion_cols].mean().reset_index()
                grouped["count"] = grouped[selected_group].map(counts)
                grouped = grouped.sort_values("count", ascending=False).head(15)

                grouped_long = grouped.melt(
                    id_vars=[selected_group, "count"],
                    value_vars=emotion_cols,
                    var_name="Emotion",
                    value_name="Mean"
                )
                grouped_long["Emotion"] = grouped_long["Emotion"].map(emotion_labels)
                ...
                fig_group.update_layout(
                    height=550,
                    xaxis_title=selected_group_label,
                    yaxis_title=ui['emotions_mean'],
                    xaxis_tickangle=-35,
        ...
        # 4. Sample table
        st.markdown("### Sample emotional records")
        ...
        show_cols = [c for c in show_cols if c in df_e.columns]

        st.dataframe(
            df_e[show_cols].head(100),
            use_container_width=True,
            hide_index=True
        )
