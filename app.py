...
def load_main_data():
    try:
        ...
        if 'ai_relevance' in df.columns:
            df = df[df['ai_relevance'].astype(str).str.lower().str.strip()
                    == 'directly_relevant'].copy()
...
# ── MAIN APP ──────────────────────────────────────────────────────────────────
def main():
    st.set_page_config(page_title="Greek Revolution Press Dashboard",
                       layout="wide")
    df = load_main_data()
    if df.empty:
        st.stop()
    st.success(f"Φορτώθηκαν {len(df):,} directly relevant εγγραφές.")
