...
LANG_UI = {
    "EL": {
        ...
        "ov_top_topics": "Top 5 Κυρίαρχα Θέματα", "ov_timeline": "📈 Εξέλιξη Όγκου Δημοσιεύσεων (Συνολικό Corpus, 1821–1832)",
        ...
        "map_title": "Συνολικές Διαδρομές Ειδήσεων", "map_legend_fr": "Από/Προς Γαλλία", "map_legend_gb": "Από/Προς Βρετανία",
    ...
    "EN": {
        ...
        "map_title": "Overall News Routes", "map_legend_fr": "From/To France", "map_legend_gb": "From/To Britain", "map_nodes": "Information Nodes",
    ...
    "FR": {
        ...
        "map_title": "Itinéraires Globaux des Informations", "map_legend_fr": "De/Vers la France", "map_legend_gb": "De/Vers la Grande-Bretagne",
...
def load_raw_timeline():
    # ΝΕΑ ΣΥΝΑΡΤΗΣΗ ΓΙΑ ΤΟ RAW FULL CORPUS TIMELINE
    path = os.path.join(BASE_DIR, "year_country_counts_full.csv")
    ...
    try:
        ...
        if 'year' in df.columns and 'year_val' not in df.columns:
            df = df.rename(columns={'year': 'year_val'})
        if 'country_analysis' in df.columns and 'country' not in df.columns:
            df = df.rename(columns={'country_analysis': 'country'})
        ...
        # Βρίσκουμε τη στήλη με το count
        count_col = next((c for c in df.columns if c in ['count', 'rows', 'n', 'total', 'volume']), None)
        if count_col and count_col != 'count':
            df = df.rename(columns={count_col: 'count'})
            
        return df
    except Exception as e:
        st.warning(f"Could not load year_country_counts_full.csv: {e}")
        return pd.DataFrame()
...
with t1:
    ...
    if not raw_timeline.empty and 'country' in raw_timeline.columns:
        ...
        # Fallback στο φιλτραρισμένο dataset αν δεν βρεθεί το αρχείο
