...
def load_waves_data():
    try:
        # Load the new slim waves file generated by the hyperscript
...
with t6:
    ...
    if df_waves.empty or not wave_cards:
        ...
        if card_data:
            ...
            # Διόρθωση του KeyError ελέγχοντας αν υπάρχει η στήλη canonical_story_cluster_id
            if "canonical_story_cluster_id" in df_waves.columns:
                df_w = df_waves[df_waves["canonical_story_cluster_id"] == cluster_id].copy()
                 ...
                 df_w = pd.DataFrame()
                 st.warning("Δεν βρέθηκε η στήλη cluster ID για το φιλτράρισμα των γραφημάτων.")
