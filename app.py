    ...
    try:
        with zipfile.ZipFile("THESIS_WITH_ORIENTATION.csv.zip", 'r') as z:
            ...
            with z.open(real_file_name) as f:
                df = pd.read_csv(f, sep=None, engine='python', encoding='utf-8-sig', on_bad_lines='skip')
    ...
    if not df_main.empty:
        ...
        with c1:
            ...
            if 'date' in df_main.columns:
                ...
                fig_year = px.histogram(df_chart, x="year_val", color="country" if 'country' in df_main.columns else None, barmode="group", color_discrete_map={'gb': '#1f77b4', 'fr': '#d62728'})
                st.plotly_chart(fig_year, use_container_width=True)
        
        with c2:
            st.subheader("⚖️ Πολιτική Στάση ανά Χώρα")
            if 'ai_stance' in df_main.columns and 'country' in df_main.columns:
                fig_sun = px.sunburst(df_main.dropna(subset=['ai_stance', 'country']), path=['country', 'ai_stance'], color='country', color_discrete_map={'gb': '#1f77b4', 'fr': '#d62728'})
...
with tab_flows:
    ...
    if not df_main.empty and 'news_origin' in df_main.columns and 'publication_place' in df_main.columns:
        ...
        fig_sankey = go.Figure(data=[go.Sankey(
            ...
            link = dict(source = top_100['news_origin'].map(node_map), target = top_100['publication_place'].map(node_map), value = top_100['counts'], color = "rgba(100, 100, 255, 0.4)")
        )])
